...
function deploy_utils() {
  ...
  class DeployHelper {
    ...
    async init(signer) {
      ...
      this.initalBalance = await hre.ethers.provider.getBalance(signer.address);
      this.currentBlockTime = (await hre.ethers.provider.getBlock()).timestamp;
      log(
        `Initial balance of deployer at ${this.address} is: ${this.initalBalance.toString()} at block timestamp : ${this.currentBlockTime
        }`,
      );
    }
